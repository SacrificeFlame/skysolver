...
export default function DataHealthWorkspace(){
 ...
 return <>
  ...
  <div className="gate-row">
   ...
   <div className={`gate ${data.deployment_allowed?'ok':'blocked'}`}>{data.deployment_allowed?<CheckCircle2/>:<ShieldCheck/>}<div><b>Publication gate</b><span>{data.deployment_allowed?'Open — recovery plan is ready for publication':'Closed — source verification or freshness checks require attention'}</span></div></div>
  </div>
  <section className="card"><header className="rowbetween"><h2>Source systems</h2><span className="muted small">evaluated {data.evaluated_at?new Date(data.evaluated_at).toLocaleTimeString():'now'}</span></header><div className="table-wrap"><table><thead><tr><th>Source</th><th>Verification</th><th>Contract</th><th>Freshness</th><th>DLQ</th><th>Drift</th><th>Circuit</th></tr></thead><tbody>{data.sources.map((s,index)=><tr key={s.source_system}><td><Database/> Operations feed {index+1}</td><td>{s.authoritative?<><CheckCircle2/> Verified</>:<><AlertTriangle/> Review required</>}</td><td>{s.contract_version.replace('fixture','schema')}</td><td>{s.fresh?`${Math.round(s.age_seconds||0)}s old`:'Snapshot'}</td><td>{s.dead_letter_count}</td><td>{s.reconciliation_drift_count}</td><td><span className={`badge ${s.circuit_state==='closed'?'success':'danger'}`}>{s.circuit_state.toUpperCase()}</span></td></tr>)}</tbody></table></div></section>
