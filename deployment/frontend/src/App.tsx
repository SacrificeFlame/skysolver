...
import{Activity,AlertTriangle,ArrowRight,Check,CloudLightning,Cpu,Database,Gauge,GitBranch,History,Map,Plane,ShieldCheck,Ticket,Users,Wrench,X}from'lucide-react';
...
type Route='overview'|'datahealth'|'disruptions'|'crew'|'flights'|'aircraft'|'routes'|'passengers'|'tiers'|'tier1'|'tier2'|'tier3'|'decisions'|'deployment'|'audit';
...
function Disruptions({d,flights,go}:{d:Disruption;flights:Flight[];go:(r:Route)=>void}){
 ...
 return <>
  ...
  <section className="card"><header className="rowbetween"><h2>Passenger connections at risk</h2><Badge tone="warning">{flights.reduce((s,f)=>s+f.connections,0)} connecting</Badge></header>
   ...
   <div className="pax-grid">{flights.map(f=>{const atRisk=f.delay>=60;const pct=Math.min(100,Math.round((f.connections/f.passengers)*100*2.4));return <div className="pax-card" key={f.id}><div className="pf"><b>{f.id}</b><span className={`badge ${atRisk?'danger':'success'}`}>{atRisk?'AT RISK':'BUFFERED'}</span></div><div className={`pax-bar ${atRisk?'risk':''}`}><span style={{width:`${pct}%`}}/></div><div className="pax-meta"><span>{f.connections} connecting</span><span>of {f.passengers} pax</span></div><p className="muted small" style={{margin:'8px 0 0'}}>{atRisk?`+${f.delay}m delay threatens ~${f.connections} onward connections`:`${f.delay}m within the connection buffer`}</p></div>})}</div></section>
 </>;
}
