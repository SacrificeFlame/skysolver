...
export default function App(){
 ...
 // Governance steps run under the role they require, then hand the session
 // back to the scheduler. The exchange is server-side: no credentials live in
 // this bundle, so it keeps working whatever the deployment's password is.
 const asRole=async<T,>(role:'duty-manager'|'deployment-controller',fn:()=>Promise<T>):Promise<T>=>{
  await api.assumeRole(role);
  try{return await fn()}finally{await api.assumeRole('scheduler-demo').catch(()=>{})}
 };
