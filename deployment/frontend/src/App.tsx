...
export default function App(){
 ...
 return <main className="app-shell"><aside className="sidebar"><div className="logo"><span><Plane/></span><div>SKYSOLVER<small>Crew recovery · India</small></div></div><nav aria-label="Primary">{navGroups.map(([group,items])=><div key={group} className="nav-group"><span className="nav-label">{group}</span>{items.map(([id,label,Icon,sub])=><button key={id} className={`${route===id||(id==='tiers'&&['tier1','tier2','tier3'].includes(route))?'active':''}${sub?' sub':''}`} aria-current={route===id?'page':undefined} onClick={()=>go(id)}><Icon/>{label}</button>)}</div>)}</nav><div className="side-foot"><Badge tone="cyan">PROTOTYPE</Badge><span>Synthetic scenario</span><small>DGCA-oriented ruleset</small></div></aside><section className="main"><TopBar/><div className="content">{page}</div></section>{notice&&<div className="toast"><ShieldCheck/>{notice}<button onClick={()=>setNotice('')}><X/></button></div>}</main>
}
