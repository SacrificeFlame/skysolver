...
export default function SolverWorkspace({selected,onOpen}:{selected?:SolverTier['id'];onOpen:(id:SolverTier['id'])=>void}){
 ...
 if(selected){
  ...
  return <>
   <header className="page-head"><div><span>{eyebrow[selected]}</span><h1>{t.name}</h1><p>{tagline[selected]}</p></div><div className="page-actions"><span className={`badge ${v.tone}`}>{v.label.toUpperCase()}</span><button onClick={load}><RefreshCw/> Run again</button></div></header>
   ...
   {selected==='tier3'&&<section className="card narrative"><h2>Human-assisted queue</h2><p>Review ranked, legality-gated options for the work automation could not resolve. Accepting an option creates a candidate — it does not approve or deploy it.</p><div className="page-actions"><button className="primary" onClick={()=>onOpen('tier3')}>Open Tier 3 queue <ArrowRight/></button></div></section>}
   <section className="card narrative"><h2><Gauge/> What this tier guarantees</h2><p>{meaning[selected]}</p><p className="run-note">This run: {t.reason.toLowerCase()} — {t.legal_assignments} legal assignments, {Math.round(t.coverage*100)}% coverage in {t.elapsed_s.toFixed(3)}s.</p></section>
   <Provline p={prov}/>
  </>;
 }
